...
class ExtensionBridge {
  ...
  private messageQueue: Map<string, (response: ScreenshotResponse) => void> =
    new Map();
        ...
        ws.on("message", (data) => {
          try {
            ...
            if (message.type === "chunk_header" && message.id) {
              ...
              console.log(
                `Starting chunked transfer for ${message.id}: ${message.totalChunks} chunks, ${message.totalSize} bytes`,
              );
            ...
            if (message.type === "chunk_data" && message.id) {
              ...
              if (buffer) {
                ...
                if (buffer.receivedChunks % 10 === 0) {
                  console.log(
                    `Received ${buffer.receivedChunks}/${buffer.totalChunks} chunks for ${message.id}`,
                  );
                }
              }
              return;
            ...
            if (message.type === "chunk_complete" && message.id) {
              ...
              if (buffer) {
                console.log(
                  `Reassembling ${buffer.totalChunks} chunks for ${message.id}`,
                );
                ...
                if (callback) {
                  console.log(
                    `Calling callback for ${message.id} with reassembled screenshot (${fullData.length} bytes)`,
                  );
                  callback({
                    success: true,
        ...
        setTimeout(() => {
          ...
          if (callback) {
            callback({
              success: false,
              error:
                "Chrome extension not connected. Please ensure the extension is loaded and refresh the page you want to capture.",
            });
          }
        }, 100);
        return;
...
export async function sendScreenshotRequest(
